...
class MPCTracker:
    ...
    result  = tracker.solve(robot_state, wildos_path, obstacle_points_2d=lidar_xy)
    ...
    # Hard-constraint radius — absolute minimum distance to any real obstacle.
    # Smaller than obs_r so it only activates when the sigmoid barrier has failed.
    # Sentinel points (1e3, 1e3) are always far enough to satisfy this trivially.
    _OBS_HARD_R = 0.35   # [m] — Go2 body half-diagonal
    ...
    def update_grid(self, grid_map: Any) -> None:
    ...
    def _build_nlp(self) -> None:
        ...
        # ── Hard distance constraint (backstop) ───────────────────────
        # Enforces a minimum physical clearance to every obstacle point.
        # Applied from step k=1 (not k=0, which is fixed to x0) so the
        # solver always has a feasible initial state.
        # Sentinels at (1e3, 1e3) satisfy dist^2 >= _OBS_HARD_R^2 trivially.
        # r2_hard = self._OBS_HARD_R ** 2
        ...
        #             (X[1, k] - p_obs[1, j]) ** 2
        #         )
        #         opti.subject_to(dist_sq >= r2_hard)
    ...
    def _build_reference(
        ...
        """
        Build an (N+1, NX) reference trajectory by advancing along the WildOS path
        at v_ref m/s from the closest waypoint, with curvature-aware arc capping.

        Curvature-aware advance (from nebula2 path_follower_node):
          The arc-length advance is capped at the first sharp corner ahead of the
          robot (consecutive segment directions dot-product < 0.5, i.e. > 60°).
          Without this, the reference would pull the robot through a corner before
          it has turned, producing a poorly conditioned reference that causes
          oscillation at turns in the WildOS graph path.

        Parameters
        ----------
        robot_state : (3,) [px, py, yaw]
        path_world  : list of (x, y[, z]) waypoints from WildOS planner
        ...
        path_xy = np.array(path_world, dtype=float)[:, :2]
        ...
        # Find closest waypoint — robot may have drifted off path during obstacle
        # avoidance; the MPC will naturally steer back.
        robot_xy  = robot_state[:2]
        distances = np.linalg.norm(path_xy - robot_xy, axis=1)
        i_closest = int(np.argmin(distances))
        s0 = arc[i_closest]

        # ── Curvature-aware arc cap ───────────────────────────────────────────
        # Walk forward from i_closest and find the first segment junction where
        # the path turns more than 60° (cos_angle < 0.5).  Cap the lookahead
        # arc at that junction so the reference never pulls the robot around a
        # sharp corner before it has reached and turned through it.
        max_arc = total_arc
        for j in range(i_closest + 1, len(path_xy) - 1):
            la = seg_len[j - 1]
            lb = seg_len[j]
            if la > 1e-6 and lb > 1e-6:
                cos_angle = float(np.dot(diffs_xy[j - 1], diffs_xy[j]) / (la * lb))
                if cos_angle < 0.5:   # > 60° turn
                    max_arc = arc[j]
                    break

        # First reference point anchors to current robot state (no discontinuity).
        x_ref[0, 0] = robot_state[0]
        x_ref[0, 1] = robot_state[1]
        x_ref[0, 2] = robot_state[2]

        for k in range(1, N + 1):
            s_k = min(s0 + v_ref * k * dt, max_arc)
