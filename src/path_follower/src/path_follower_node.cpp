...
private:
  ...
  void odom_callback(const nav_msgs::msg::Odometry::ConstSharedPtr msg)
  {
    ...
    try
    {
      odom_pose_in_path_frame =
        tf_buffer_.transform(odom_pose, path_->header.frame_id, tf2::durationFromSec(0.1));
    }
    catch (const tf2::TransformException & ex)
    {
      RCLCPP_WARN(
        this->get_logger(), "Could not transform odometry pose to path frame: %s", ex.what());
      return;
    }

    Eigen::Vector3d robot_pos(
      odom_pose_in_path_frame.pose.position.x,
      odom_pose_in_path_frame.pose.position.y,
      odom_pose_in_path_frame.pose.position.z);

    std::vector<Eigen::Vector3d> path_points;
    std::vector<double> path_robot_distances;
    path_points.reserve(path_->poses.size());
    path_robot_distances.reserve(path_->poses.size());
    ...
    const size_t closest_index =
      static_cast<size_t>(std::min_element(
        path_robot_distances.begin(),
        path_robot_distances.end()) - path_robot_distances.begin());

    // If the last waypoint is already the closest one, publish it directly.
    ...
    {
      const Eigen::Vector3d current_delta = path_points[wp_index] - path_points[wp_index - 1];
      const Eigen::Vector3d next_delta = path_points[wp_index + 1] - path_points[wp_index];

      if (current_delta.norm() < 1e-9 || next_delta.norm() < 1e-9)
      {
        break;
      }

      const Eigen::Vector3d current_dir = current_delta.normalized();
      const Eigen::Vector3d next_dir = next_delta.normalized();
    ...
    const double a = path_robot_distances[wp_index - 1] - wp_lookahead_dist_;
    const double b = path_robot_distances[wp_index] - wp_lookahead_dist_;

    double wp_fraction = 0.0;
    if (std::abs(b - a) > 1e-9)
    {
      wp_fraction = -a / (b - a);
    }
    ...
    pose.pose.position.x = path_->poses[wp_index].pose.position.x * wp_fraction +
      path_->poses[wp_index - 1].pose.position.x * (1.0 - wp_fraction);
    pose.pose.position.y = path_->poses[wp_index].pose.position.y * wp_fraction +
      path_->poses[wp_index - 1].pose.position.y * (1.0 - wp_fraction);
    pose.pose.position.z = path_->poses[wp_index].pose.position.z * wp_fraction +
      path_->poses[wp_index - 1].pose.position.z * (1.0 - wp_fraction);
